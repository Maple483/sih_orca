(() => {
  ...
  const num = n => Number.isFinite(Number(n)) ? new Intl.NumberFormat('en-IN',{maximumFractionDigits:1}).format(Number(n)) : '—';
  const status = d => d === 'increasing' ? 'Increasing' : d === 'decreasing' ? 'Decreasing' : 'Broadly stable';
  const arrow = d => d === 'increasing' ? '↑' : d === 'decreasing' ? '↓' : '→';
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      .mp-brand{display:flex;align-items:center;gap:12px}.mp-icon{width:40px;height:40px;border-radius:10px;background:rgba(16,185,129,.12);border:1px solid rgba(16,185,129,.3);display:grid;place-items:center;font-size:21px}.mp-title{font-weight:800;font-size:18px}.mp-sub{font-size:10px;color:#6ee7b7;margin-top:2px;text-transform:uppercase;letter-spacing:.12em}
  ...
  function lineChart(rows,key,unit){
    ...
    const circles=rows.map((r,i)=>`<circle cx="${x(i)}" cy="${y(Number(r[key])||0)}" r="5"><title>${esc(r.Year)}: ${num(r[key])} ${unit}</title></circle>`).join('');
    const labels=rows.map((r,i)=>`<text x="${x(i)}" y="${H-16}" text-anchor="middle" class="year">${r.Year}</text>`).join('');
    return `<div class="mp-scroll"><svg viewBox="0 0 ${W} ${H}" class="mp-svg"><g class="grid">${grid}</g><line class="axis" x1="${L}" y1="${T}" x2="${L}" y2="${H-B}"/><line class="axis" x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}"/><polyline class="line" points="${pts}"/>${circles}${labels}</svg></div><div class="mp-legend">Hover a point for the exact value.</div>`;
  }
  function barChart(rows,key,labelKey){
    ...
    const bars=rows.map((r,i)=>{const v=Number(r[key])||0,h=v/max*(base-T),bx=L+(i+.5)*(W-L-R)/rows.length-bw/2;return `<g><rect x="${bx}" y="${base-h}" width="${bw}" height="${h}" rx="4" fill="#10b981"><title>${esc(r[labelKey])}: ${num(v)} tonnes</title></rect><text x="${bx+bw/2}" y="${H-28}" text-anchor="middle" fill="#94a3b8" font-size="10">${esc(String(r[labelKey]).length>14?String(r[labelKey]).slice(0,14)+'…':r[labelKey])}</text></g>`}).join('');
    const grid=Array.from({length:6},(_,i)=>{const v=max*i/5,yy=base-(v/max)*(base-T);return `<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}" stroke="#1e293b"/><text x="${L-8}" y="${yy+4}" text-anchor="end" fill="#64748b" font-size="10">${num(v)}</text>`}).join('');
    return `<div class="mp-scroll"><svg viewBox="0 0 ${W} ${H}" class="mp-svg"><g>${grid}</g>${bars}</svg></div><div class="mp-legend">Hover a bar for the exact value.</div>`;
  }
  ...
  async function openEnhanced(){
    ...
    root.innerHTML=`<div class="mp-shell"><header class="mp-head"><div class="mp-brand"><div class="mp-icon">🐟</div><div><div class="mp-title">Marine Productivity</div><div class="mp-sub">Historical analytics · Potential Fishing Zone decision support</div></div></div><button class="mp-back" id="mp-close">← Back to Dashboard</button></header><div class="mp-tabs"><button class="mp-tab active" data-tab="trends">Past Trends<small>Historical catch, SST, chlorophyll & species analytics</small></button><button class="mp-tab" data-tab="pfz">PFZ Finder<small>Recommended fishing zones + live marine conditions + satellite maps</small></button></div><main class="mp-main" id="mp-body"></main></div>`;
    ...
    async function renderTrends(){
      ...
      rs.onchange=async()=>{region=rs.value;const sp=await loadSpecies();ss.innerHTML='<option value="">All fish / total catch</option>'+sp.map(s=>`<option>${esc(s)}</option>`).join('');await run();};ss.onchange=run;body.querySelector('#mp-run').onclick=run;await run();
    }
    function renderAnalysis(a,e,tc){
      const cs=a.correlation.catch_vs_sst,cc=a.correlation.catch_vs_chlorophyll;
      tc.innerHTML=`
        <div class="mp-card mp-summary"><div class="mp-summary-top"><div><h3>${esc(a.state)} ${a.species_filter?`· ${esc(a.species_filter)}`:''}</h3><div style="font-size:12px;color:#94a3b8">${a.species_filter?`Selected species catch is ${status(a.explanation.direction).toLowerCase()}.`:`Total fish catch is ${status(a.explanation.direction).toLowerCase()} over 2007–2012.`}</div></div><span class="mp-pill">${arrow(a.explanation.direction)} ${status(a.explanation.direction)}</span></div><p style="font-size:12px;color:#cbd5e1;line-height:1.6;margin:12px 0 0">${esc(a.explanation.text)}</p><div class="mp-note">⚠ Historical SST/chlorophyll values for 2007–2012 are synthetic and are retained for historical visualization only.</div></div>
        <div class="mp-grid2"><section class="mp-card"><h3>Catch ↔ SST</h3><div class="mp-big">${cs==null?'—':cs.toFixed(2)}</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">${esc(a.explanation.sst_effect||'No clear association')}</div></section><section class="mp-card"><h3>Catch ↔ Chlorophyll-a</h3><div class="mp-big">${cc==null?'—':cc.toFixed(2)}</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">${esc(a.explanation.chlorophyll_effect||'No clear association')}</div></section></div>
        <section class="mp-card"><h3>${esc(a.species_filter||'Total')} Annual Fish Catch (tonnes)</h3>${lineChart(a.annual,'catch','tonnes')}</section>
        <section class="mp-card"><h3>SST & Chlorophyll Trends</h3>${lineChart(a.annual,'sst','°C')}</section>
        <section class="mp-card"><h3>Seasonal Mean Catch</h3>${barChart(a.seasonal,'mean_catch','Season')}</section>
        <section class="mp-card"><h3>${a.species_filter?`Catch distribution for ${esc(a.species_filter)}`:'Top Species by Catch'}</h3>${barChart(a.top_species,'catch_tonnes','Species')}</section>
        <section class="mp-card"><h3>Catch anomalies</h3>${a.anomalies?.length?`<table class="mp-table"><thead><tr><th>Year</th><th>Catch</th><th>Z-score</th></tr></thead><tbody>${a.anomalies.map(x=>`<tr><td>${x.Year}</td><td>${num(x.catch)} t</td><td>${Number(x.catch_z).toFixed(2)}</td></tr>`).join('')}</tbody></table>`:'<div style="font-size:12px;color:#64748b">No catch anomalies detected under the current rule.</div>'}</section>`;
    }
    ...
    function renderPFZData(d,lat,lon){
      ...
      results.forEach((r,i)=>{const best=i===0;const icon=window.L.divIcon({className:'pfz-advisory-icon',html:`<div style="width:${best?24:18}px;height:${best?24:18}px;border-radius:50%;background:${best?'#34d399':'#facc15'};border:3px solid white;box-shadow:0 0 12px ${best?'rgba(52,211,153,.7)':'rgba(250,204,21,.5)'};display:grid;place-items:center;color:#111;font-weight:900;font-size:9px">${r.rank||i+1}</div>`,iconSize:[best?24:18,best?24:18],iconAnchor:[best?12:9,best?12:9]});window.L.marker([r.lat,r.lon],{icon}).addTo(pfzMap).bindPopup(`<b>PFZ #${r.rank||i+1}: ${esc(r.from_coast)}</b><br>Score: ${r.rank_score}/100<br>Distance from you: ${r.distance_km} km<br>Direction: ${esc(r.direction||'—')} · Bearing: ${num(r.bearing_deg)}°<br>Advisory distance: ${esc(r.distance_advisory_km)} km<br>Depth: ${esc(r.depth_m)} m<br>${esc(r.forecast_validity)}<br>${esc((r.reasons||[]).join(', '))}`)});
    ...
    const ok=await loadRegions();if(ok)await renderTrends();
  }
