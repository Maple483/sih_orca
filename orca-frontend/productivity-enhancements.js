(() => {
  ...
  function barChart(rows,key,labelKey){
    ...
    const grid=Array.from({length:5},(_,i)=>{const v=max*i/4,yy=base-(v/max)*(base-T);return `<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-8}" y="${yy+4}" text-anchor="end">${num(v)}</text>`}).join('');
  ...
  async function openEnhanced(){
    ...
    const root=document.createElement('div');root.id='orca-productivity-enhanced';root.innerHTML=`<div class="pe-shell"><div class="pe-head"><div><div class="pe-title">🐟 Marine Productivity</div><div class="pe-sub">Landings × SST × chlorophyll · 2007–2012</div></div><button id="pe-close">← Back to Dashboard</button></div><div class="pe-controls"><label>Coastal region<select id="pe-region"><option>Loading…</option></select></label><label>Species<select id="pe-species"><option value="">All fish / total catch</option></select></label><button id="pe-run">Run analysis</button><span id="pe-loading"></span></div><div id="pe-content" class="pe-content"><div class="pe-empty">Select a region to load the analysis.</div></div></div>`;document.body.appendChild(root);
    ...
    function render(a,e){
      const direction=status(a.explanation.direction), corrS=a.correlation.catch_vs_sst, corrC=a.correlation.catch_vs_chlorophyll;
      const relationship=v=>v==null?'No clear relationship':v>=.3?'Positive association':v<=-.3?'Negative association':'No clear association';
      content.innerHTML=`
        <div class="pe-summary"><div><b>${esc(a.state)}</b>${a.species_filter?`<span> · ${esc(a.species_filter)}</span>`:''}</div><span class="pe-pill">${direction}</span><p>${esc(a.explanation.text)}</p><div class="pe-effects"><div><b>SST</b><br>${esc(a.explanation.sst_trend)} · ${relationship(corrS)}</div><div><b>Chlorophyll-a</b><br>${esc(a.explanation.chlorophyll_trend)} · ${relationship(corrC)}</div></div><small>${esc(a.explanation.caution)}</small></div>
        ${card(`${esc(a.species_filter||'Total')} annual catch — exact value on hover`,lineChart(a.annual,'catch','Catch','tonnes'))}
        ${card('Seasonal catch — exact value on hover',barChart(a.seasonal,'mean_catch','Season'))}
        ...
        <div class="pe-two">${card('Catch ↔ SST',`<div class="pe-big">${corrS==null?'—':corrS.toFixed(2)}</div><div>${relationship(corrS)}</div>`)}${card('Catch ↔ Chlorophyll-a',`<div class="pe-big">${corrC==null?'—':corrC.toFixed(2)}</div><div>${relationship(corrC)}</div>`)}</div>
        ${card('Plain-language takeaway',`<p class="pe-text">${esc(a.explanation.text)}</p><p class="pe-note">${esc(a.explanation.caution)}</p>`)}
      `;
    }
